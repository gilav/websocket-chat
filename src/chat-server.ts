...
export class ChatServer {
  ...
  private io: socketIo;
  private sysIcon = gravatar.url('', { s: '140', r: 'x', d: 'retro' });

  // map of AV status: enables, disabled
  private peoplesAvAvailableMap: any = new Map();
  private peoplesAvDisabledMap: any = new Map();
  ...
  private listen(): void {
    ...
    this.io.on('connection', socket => {
      let address: any = socket.handshake.address;
      ...
      socket.on(
        ...
        function(data) {
          ...
          if (data.type == 'avEnabled') {
            ...
            socket.emit('sys', data);
            let destSocket: socketIo.Socket = this.getSocketFromUsername(socket.room, data.receiver);
            console.log(
              '  >>> initiateVideoChat; destSocket=' + destSocket + '; socket.username:' + destSocket.username,
            );
            ...
            let destSocket: socketIo.Socket = this.getSocketFromUsername(socket.room, data.sender);
  ...
  //
  private handleLoad(socket, data) {
    if (this.DEBUG) {
      console.log(' !! handleLoad; socket id:' + socket.id + '; socket room:' + socket.room + '; data:' + data);
    }

    //
    let socketsInRoom: socketIo.Socket[] = this.getConnectedSocketInRoom(data);
  ...
  private handleLogin(socket, data) {
    ...
    // find people in room
    //
    const socketsInRoom: socketIo.Socket[] = this.getConnectedSocketInRoom(data.id);
    if (this.DEBUG) {
    ...
    if (socketsInRoom.length > 10) {
      ...
      console.log(
        ' ############################ updateAvMaps returns:' + avPossibleList + '; length:' + avPossibleList.length,
      );

      // bradcast usersOnline to everybody
      console.log(
        ' <handleLogin.emit peoplesList usersOnline=' + usersOnline + '; length usersOnline=' + usersOnline.length,
      );
  ...
  private handleDisconnect(socket, data) {
    ...
    // find socket in room
    //
    let socketsInRoom: socketIo.Socket[] = this.getConnectedSocketInRoom(socket.room);
    if (this.DEBUG) {
      console.log('  login; found socketsInRoom=' + socketsInRoom + '; length=' + socketsInRoom.length);
    }
    // and get peoples
    ...
    // update the av maps
    let avPossibleList: string[] = this.updateAvMaps(socketsInRoom);
    console.log(
      ' ############################ updateAvMaps returns:' + avPossibleList + '; length:' + avPossibleList.length,
    );
  ...
  private getConnectedSocketInRoom(roomId): socketIo.Socket[] {
    console.log(' getConnectedIdsInRoom: roomId:' + roomId);

    let roomConnectedSockets: socketIo.Socket[] = [];
    let namespace: socketIo.NameSpace = this.io.of('/'); // the default namespace is "/chat"
    ...
    if (namespace) {
      ...
      for (let id in namespace.connected) {
        ...
        if (roomId) {
          let aSocket: socketIo.Socket = namespace.connected[id];
      ...
      if (n == 0) {
        ...
        console.log(
          '  getConnectedIdsInRoom: ' + roomConnectedSockets.length + ' people(s) connected to this namespace room',
        );
        n = 0;
  ...
  private getSocketFromUsername(roomId, name): socketIo.Socket {
    console.log(' getSocketFromUsername: name:' + name);
    let aSocket: socketIo.Socket = undefined;
    let namespace: socketIo.NameSpace = this.io.of('/'); // the default namespace is "/chat"
  ...
  private getIdenticonUrl(email: string): string {
  ...
  private updateAvMaps(peoplesSocket): string[] {
    ...
    let avPossibleList: string[] = [];

    let theKeys: string[] = Array.from(this.peoplesAvAvailableMap.keys());
    theKeys.forEach(
      function(value) {
      ...
      function(value) {
        if (thePeoples.includes(value)) {
          if (this.peoplesAvDisabledMap.get(value) == false) {
            const index: number = avPossibleList.indexOf(value, 0);
            if (index > -1) {
